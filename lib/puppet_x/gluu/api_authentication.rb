module PuppetX
  module Gluu::ApiAuthentication
    include PuppetX::Gluu::Helpers::Ldap

    def self.configure(key_pass, ldap_pass, oxtrust_admin_pass)
      ...
      # Get the generated oxAuth JKS password from the encrypted setup
      # properties file.
    ...
    LOGIN
      ...
      puppet_error(stderr) unless exit_status.to_i.zero?
    ...
    def self.get_jks_pass(oxtrust_admin_pass)
      cmd = <<~CMD
      openssl enc -d -aes-256-cbc \
        -in /install/community-edition-setup/setup.properties.last.enc \
        -pass pass:#{oxtrust_admin_pass} \
        | grep -Po '(?<=api_rp_client_jks_pass=).*$'
    CMD

      run_container_command(cmd).strip
    end
